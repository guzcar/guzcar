<x-pdf-layout title="Presupuesto {{ $trabajo->codigo }}" code="{{ $trabajo->codigo }}" tipoReporte="PRESUPUESTO">
    ...
    <table class="table-void">
        <tbody>
            ...
            <tr>
                ...
                <td style="border-bottom: dotted black 1px;">{{ optional($trabajo?->fecha_salida)->format('d/m/Y') ?? '' }}</td>
            </tr>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            {{-- Iteramos sobre la colección unificada y ordenada --}}
                ...
                <tr>
                    ...
                    <td>
                        @if($item->tipo_item === 'articulo')
                            ...
                            @endphp
                        @else
                            {{-- Si es 'otro', mostramos la descripción directa --}}
                            {{ $item->descripcion }}
                        @endif
                    </td>
                    ...
                    {{-- Precio y Subtotal (Igual para ambos) --}}
                    <td class="text-right">S/ {{ $item->precio }}</td>
                    <td class="text-right">S/ {{ number_format($item->cantidad * $item->precio, 2) }}</td>
                </tr>
            @empty
                <tr>
        ...
        <tfoot>
            <tr>
                ...
                {{-- Los subtotales se siguen pasando desde el controlador, así que esta suma sigue funcionando --}}
                <td class="text-right border-top bold">S/ {{ number_format($subtotal_articulos + $subtotal_trabajo_otros, 2) }}</td>
            </tr>
        </tfoot>
    </table>
                        ...
                        <tbody>
                            ...
                            @foreach($descuentos as $descuento)
                                <tr>
                                    ...
                                    <td class="text-right">S/ {{ number_format($total * ($descuento->descuento / 100), 2) }}</td>
                                </tr>
                            @endforeach
                            ...
                            @if ($trabajo->igv)
                                <tr>
                                    <td>IGV (18%)</td>
                                    <td class="text-right">S/ {{ number_format($total_con_descuentos * 0.18, 2) }}</td>
                                </tr>
                            @endif
                        </tbody>
                        <tfoot>
                            <tr>
                                ...
                                <td class="text-right border-top bold">
                                    S/ {{ number_format($total_con_igv = $total_con_descuentos * (1 + ($trabajo->igv ? 0.18 : 0)), 2) }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
