...
<head>
    ...
    <title>Informe {{ $trabajo->codigo }}</title>
    <style>
        @page {
            size: A4;
...
<body>
            ...
            <tbody>
                <tr style="height: 100%;">
                    ...
                    <td class="text-left" style="vertical-align: middle; padding-right: 1rem;">
                        ...
                        <p class="m-0"></p>
                        <p class="m-0"><b>INFORME TÉCNICO N° {{ $trabajo->codigo }}</b></p>
                        <p class="m-0">{{ $fechaSalida }}</p>
                    </td>
    ...
    @foreach($informes as $informe)
        <div style="font-size: 13px; text-align: justify; text-justify: inter-word; text-transform: none; margin-bottom: 15px;">
            {!! $informe->contenido !!}
        </div>
    @endforeach

    <!-- Sección Inferior: Firma Centrada -->
    @if($firmaBase64)
        <div style="margin-top: 40px; text-align: center; page-break-inside: avoid;">
            <!-- Ajusta el width si la firma sigue siendo muy grande o pequeña -->
            <img src="{{ $firmaBase64 }}" alt="Firma" style="width: 250px; max-width: 100%; height: auto; display: inline-block;">
        </div>
    @endif
    </div>
</body>
