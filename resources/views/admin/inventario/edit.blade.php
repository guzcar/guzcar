...
<head>
    ...
    <style>
        ...
        .unsaved-changes-modal {
            z-index: 1001; /* Mayor que el modal de eliminación */
        }
    ...
    <div id="unsavedChangesModal" class="modal-overlay unsaved-changes-modal">
        <div class="modal-content">
            <div class="flex items-start mb-4">
                <svg class="warning-icon w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                </svg>
                <div>
                    <h3 class="text-lg font-bold text-gray-800">Cambios sin guardar</h3>
                    <p class="text-gray-600 mt-1">Tienes cambios sin guardar en el inventario. Si sales ahora, perderás todos los cambios.</p>
                </div>
            </div>
            <div class="flex justify-end gap-2">
                <button id="cancelExit" class="px-4 py-2 bg-gray-300 rounded-md hover:bg-gray-400">Permanecer</button>
                <button id="confirmExit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Salir sin guardar</button>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <form action="{{ route('admin.trabajos.inventario.update', ['trabajo' => $trabajo]) }}" method="POST" id="inventarioForm">
                ...
                <div class="flex flex-col gap-6">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div class="search-container">
                            <input type="text" id="checklist-search" class="w-full border border-gray-200 rounded-md px-3 py-2 shadow-sm" placeholder="Buscar items...">
                        </div>
                        
                        <div id="checklist-container" class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                            @foreach($defaultItems as $itemNombre)
                                @php
                                    $itemSaved = $savedChecklist->firstWhere('nombre', $itemNombre) ?? ['nombre' => $itemNombre, 'checked' => false];
                                    $index = 'default_' . $loop->index;
                                @endphp
                                <div class="checklist-item flex items-center" data-item-name="{{ strtolower($itemNombre) }}">
                                    <input type="checkbox" name="checklist[{{ $index }}][checked]" value="1" {{ ($itemSaved['checked'] ?? false) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <input type="hidden" name="checklist[{{ $index }}][nombre]" value="{{ $itemNombre }}">
                                    <label class="ml-3 block font-medium text-gray-700">{{ $itemNombre }}</label>
                                </div>
                            @endforeach

                            @foreach($savedChecklist->whereNotIn('nombre', $defaultItems) as $item)
                                @php $index = 'new_' . $loop->index; @endphp
                                <div class="checklist-item-new flex items-center col-span-1" data-item-name="{{ strtolower($item['nombre']) }}">
                                    <!-- CORRECCIÓN: Sin checkbox para items nuevos -->
                                    <input type="hidden" name="checklist[{{ $index }}][checked]" value="1">
                                    <input type="text" name="checklist[{{ $index }}][nombre]" value="{{ $item['nombre'] }}" class="flex-1 border border-gray-200 rounded-md px-2 py-1 shadow-sm" required>
                                    <button type="button" class="remove-checklist ml-2 text-2xl text-red-500 hover:text-red-700 transition-colors" title="Eliminar item">&times;</button>
                                </div>
                            @endforeach
                        </div>
                        
                        <div class="flex justify-end mt-5">
                            <button type="button" id="add-checklist" class="flex items-center pr-4 pl-2 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 text-sm font-semibold transition-colors">
                            ...
                            <div class="flex justify-around">
                                <div class="text-center flex flex-col items-center">
                                    ...
                                    <div class="level-slider-container">
                                        <div class="labels"><span>F</span><span>¾</span><span>½</span><span>¼</span><span>E</span></div>
                                        <input type="range" min="0" max="100" value="{{ $inventarioData['combustible'] ?? 50 }}" name="combustible" class="level-slider" orient="vertical">
                                    </div>
                                    <div class="mt-2 font-semibold level-value" id="combustible-value">{{ $inventarioData['combustible'] ?? 50 }}%</div>
                                </div>
                                <div class="text-center flex flex-col items-center">
                                    <label class="block font-medium mb-2">Aceite</label>
                                    <div class="level-slider-container">
                                       <div class="labels"><span>F</span><span>¾</span><span>½</span><span>¼</span><span>E</span></div>
                                        <input type="range" min="0" max="100" value="{{ $inventarioData['aceite'] ?? 50 }}" name="aceite" class="level-slider" orient="vertical">
                                    </div>
                                    <div class="mt-2 font-semibold level-value" id="aceite-value">{{ $inventarioData['aceite'] ?? 50 }}%</div>
                                </div>
                            </div>
                            <div class="sm:col-span-1">
                                <label for="observaciones" class="block font-medium mb-2">Observaciones</label>
                                <textarea name="observaciones" id="observaciones" rows="8" class="w-full border border-gray-200 rounded-md px-3 py-2 shadow-sm" placeholder="Anotaciones generales...">{{ $inventarioData['observaciones'] ?? '' }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        @if($trabajo->vehiculo?->tipoVehiculo?->diagrama)
                            <div class="flex flex-wrap items-center gap-4 mb-4">
                                <div>
                                    ...
                                    <select id="symbol-select" class="border border-gray-300 rounded-md shadow-sm px-3 py-2">
                                ...
                                <div>
                                    ...
                                    <div class="inline-flex rounded-md shadow-sm">
                                        <button type="button" data-mode="draw" class="mode-btn active px-3 py-2 border border-gray-300 rounded-l-md">Dibujar</button>
                                        <button type="button" data-mode="edit" class="mode-btn px-3 py-2 border-t border-b border-gray-300">Editar</button>
                                        <button type="button" data-mode="delete" class="mode-btn px-3 py-2 border border-gray-300 rounded-r-md">Eliminar</button>
                                    </div>
                                </div>
                            </div>

                            <div id="diagram-canvas">
                                <img id="vehicle-image" src="{{ asset('storage/' . $trabajo->vehiculo->tipoVehiculo->diagrama) }}" alt="Diagrama del vehículo">
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div class="w-full max-w-sm mx-auto">
                            ...
                            <div class="flex justify-between items-center mt-2">
                                <button type="button" id="clearSignature" class="text-gray-600 hover:text-gray-800 text-xs">Limpiar Firma</button>
            ...
            <div class="flex justify-end space-x-4 mt-8">
                <button type="button" id="cancelButton" class="px-6 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors font-medium">
                    Cancelar
                </button>
    ...
    <script>
        ...
        const setupChangeDetection = () => {
            ...
            document.addEventListener('click', function(e) {
                if (e.target.matches('#add-checklist, .remove-checklist, #clearSignature, .mode-btn')) {
                    markFormChanged();
                }
            });
        ...
        function addChecklistItem() {
            ...
            newItem.className = 'checklist-item-new flex items-center col-span-1';
