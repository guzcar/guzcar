    ...
    <ul class="navbar-nav ms-auto me-3 me-lg-4">
        ...
        <li class="nav-item dropdown me-3">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow p-0" aria-labelledby="navbarDropdownNotifications"
                ...
                <li class="dropdown-header bg-light fw-bold border-bottom">Avisos Globales</li>

                @if(isset($globalEvents) && $globalEvents->count() > 0)
                    @foreach($globalEvents as $event)
                        <li>
                            <div class="dropdown-item p-3 border-bottom" style="white-space: normal;">
                                <h6 class="mb-1 text-primary fw-bold">{{ $event->title }}</h6>
                                <p class="mb-1 small text-muted">
                                    {{ Str::limit(strip_tags($event->description ?? 'Sin descripción adicional'), 80) }}</p>
                                <small class="text-secondary" style="font-size: 0.75rem;">
                                    <i class="far fa-clock me-1"></i> Hasta: {{ $event->ends_at->format('d/m/Y H:i') }}
                                </small>
                            </div>
                        </li>
                    @endforeach
                @else
                    <li><span class="dropdown-item text-muted text-center py-3">No hay avisos activos</span></li>
                @endif
            </ul>
        </li>
...
@if(isset($globalEvents) && $globalEvents->count() > 0)
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="globalEventModalLabel">
                        <i class="fas fa-bullhorn me-2"></i> Comunicado Importante
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body p-4">
                    <h4 class="text-dark fw-bold mb-3">{{ $latestEvent->title }}</h4>
                    ...
                    <div class="alert alert-light border d-flex align-items-center" role="alert">
                        ...
                        <div>
                            Vigente hasta: <strong>{{ $latestEvent->ends_at->format('d/m/Y h:i A') }}</strong>
                        </div>
                    </div>

                    @if($globalEvents->count() > 1)
                        <p class="text-muted small text-end mt-2 fst-italic">
                            Tienes {{ $globalEvents->count() - 1 }} avisos más en la campana.
                        </p>
                    @endif
                </div>
                <div class="modal-footer bg-light">
                    <button type="button" class="btn btn-primary w-100" data-bs-dismiss="modal"
                        id="btnAcknowledge">Entendido</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (modalElement) {
                var eventId = modalElement.getAttribute('data-event-id');
